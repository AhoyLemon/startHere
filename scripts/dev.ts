...
function displaySuccess(spinner: any) {
  ...
  console.log('\n');
  console.log(chalk.yellow('  🍋 ') + chalk.bold.white(`${projectName} is running locally\n`));
  ...
  console.log(chalk.dim('  Press ') + chalk.white('Ctrl+C') + chalk.dim(' to stop\n'));
}
...
function startProcess(
  ...
  proc.stdout.on('data', (data: Buffer) => {
    ...
    if (name === 'Sass') {
      if (!sassReady && (output.includes('Compiled') || output.includes('Sass is watching') || output.includes('watching for changes'))) {
        sassReady = true;
        spinner.text = `${chalk.green('✓')} Sass ready`;
        checkAllReady(spinner);
      }
    } else if (name === 'Pug') {
      if (!pugReady && (output.includes('Watching') || output.includes('👀') || output.includes('Building'))) {
        pugReady = true;
        spinner.text = `${chalk.green('✓')} Pug ready`;
        checkAllReady(spinner);
      }
    } else if (name === 'TypeScript') {
      if (!tsReady && (output.includes('Watching') || output.includes('Found 0 errors') || output.includes('Starting compilation') || output.includes('File change detected'))) {
        tsReady = true;
        spinner.text = `${chalk.green('✓')} TypeScript ready`;
        checkAllReady(spinner);
      }
    } else if (name === 'BrowserSync') {
      if (!browserSyncReady && (output.includes('Local:') || output.includes('Serving files') || output.includes('Access URLs'))) {
  ...
  proc.stderr.on('data', (data: Buffer) => {
    ...
    if (name === 'TypeScript' && !tsReady && (output.includes('Watching') || output.includes('Starting compilation'))) {
      tsReady = true;
      spinner.text = `${chalk.green('✓')} TypeScript ready`;
      checkAllReady(spinner);
      return;
...
async function startDev() {
  ...
  // Start all processes
  startProcess('Sass', 'sass', ['scss/site.scss:css/site.css', '--no-source-map', '--watch'], spinner);
  ...
  setTimeout(() => {
    if (!browserSyncReady || !sassReady || !pugReady || !tsReady) {
      spinner.warn(chalk.yellow('Some services are taking longer than expected...'));
    }
  }, 10000);
}
